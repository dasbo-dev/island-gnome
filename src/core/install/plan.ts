 ...
 * bit stops being load-bearing: the only chmod that adds the bit lives in the
 * Makefile's install target, which local installs run and the packaged path
 * never does, and build.mjs deliberately strips it from the packaged copy
 * instead — a dropped mode would make every hook fail silently. Bare `gjs`, not
